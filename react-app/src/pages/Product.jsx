          ...
          <>
            <div className="flex flex-col md:flex-row md:flex-1 mx-auto gap-5 ">
              <div className="hidden md:flex md:flex-col md:flex-1 max-w-[150px]">
                <img className="w-full h-[150px] w-[150px] hidden lg:flex object-cover cursor-pointer mb-[10px] ml-[5px] border border-solid border-gray border-1"src={process.env.REACT_APP_UPLOAD_URL + data?.attributes?.img?.data?.attributes?.url} alt="" onClick={(e) => setSelectedImg("img")}/>
                <img className="w-full h-[150px] w-[150px] hidden lg:flex object-cover cursor-pointer mb-[10px] ml-[5px] border border-solid border-gray border-1" src={process.env.REACT_APP_UPLOAD_URL + data?.attributes?.img2?.data?.attributes?.url} alt="" onClick={(e) => setSelectedImg("img2")} onError = {e => e.target.style.display = 'none'}/>
                <img className="w-full h-[150px] w-[150px] hidden lg:flex object-cover cursor-pointer mb-[10px] ml-[5px] border border-solid border-gray border-1" src={process.env.REACT_APP_UPLOAD_URL + data?.attributes?.img3?.data?.attributes?.url} alt="" onClick={(e) => setSelectedImg("img3")} onError = {e => e.target.style.display = 'none'}/>
                <img className="w-full h-[150px] w-[150px] hidden lg:flex object-cover cursor-pointer mb-[10px] ml-[5px] border border-solid border-gray border-1" src={process.env.REACT_APP_UPLOAD_URL + data?.attributes?.img4?.data?.attributes?.url} alt="" onClick={(e) => setSelectedImg("img4")} onError = {e => e.target.style.display = 'none'}/>
              </div>
              <div className="relative lg:w-2/3">
                    ...
                    <button onClick={(e) => setSelectedImg("img2")} className="absolute right-1 bottom-1/2 flex lg:hidden" onError = {e => e.target.style.display = 'none'}><FaArrowRight className="hover:bg-gray-100 w-[60px] h-[60px] p-3 bg-gray-50/50"/></button>
                        <img className="flex w-full min-h-[200px] md:min-h-[700px] max-h-[1200px] md:max-h-[600px] object-contain max-w-[700px]"
                        src={
